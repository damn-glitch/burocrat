...
export default function EditProject() {
    ...
    // Пример заполненных данных проекта
    const [projectName, setProjectName] = useState('Burokrat app design');
    const [projectDescription, setProjectDescription] = useState('Дизайн мобильного приложения для управления задачами и проектами.');
    const [selectedColor, setSelectedColor] = useState('#8B5CF6');
    const [selectedParticipantIds, setSelectedParticipantIds] = useState(['1', '2']);
    const [invitedParticipants, setInvitedParticipants] = useState([
        {
            id: 'invited-1',
            name: 'Alexey',
            email: 'alexey@example.com',
            avatar: 'https://ui-avatars.com/api/?name=Alexey&background=random',
            role: 'Тестировщик',
            source: 'invited',
        },
    ]);
    ...
    const [files, setFiles] = useState([
        {
            name: 'ТЗ.pdf',
            size: 1024 * 1024 * 2,
            uri: 'https://example.com/tz.pdf',
        },
        {
            name: 'макет.fig',
            size: 1024 * 500,
            uri: 'https://example.com/maket.fig',
        },
    ]);

    const [period, setPeriod] = useState<PeriodValue>({ start: '2024-10-01', end: '2024-12-31' });
    const [showCalendarModal, setShowCalendarModal] = useState(false);

    const participantsPreview = useMemo(
        () => [
            ...COMPANY_PARTICIPANTS.filter((participant) =>
    ...
    const handleAddInvitedParticipant = (email: string, role: string) => {
        ...
        const name =
            nameCandidate.charAt(0).toUpperCase() + nameCandidate.slice(1);
    ...
    const handlePickFile = async () => {
        ...
        if (!result.canceled && result.assets && result.assets.length > 0) {
            const mappedAssets = result.assets
                .filter(asset => asset.name && asset.size !== undefined && asset.uri)
                .map(asset => ({
                    name: asset.name!,
                    size: asset.size!,
                    uri: asset.uri!
                }));
            setFiles((prev) => [...prev, ...mappedAssets]);
        }
    };
    ...
    const participantsForDisplay = useMemo(
        () => [
            ...COMPANY_PARTICIPANTS.map(p => ({ ...p, source: "company" as const })),
            ...invitedParticipants.map(p => ({ ...p, source: "invited" as const }))
        ],
        [invitedParticipants]
    );
    ...
    return (
        <SafeAreaView style={styles.safeArea} edges={['top', 'bottom']}>
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 20 }}>
                <Header
                    title={t('projectForm.headerEdit')}
                    titleFontSize={16}
                    backButton
                ...
                <Text style={styles.pageTitle}>{t('projectForm.mainTitleEdit')}</Text>
                ...
                <View style={styles.saveButtonContainer}>
                    <PrimaryButton label={t('projectForm.saveButtonEdit')} onPress={handleSaveProject} />
                </View>
            </ScrollView>
